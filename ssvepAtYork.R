...
eegDat <- read.csv(filesEEG[2], stringsAsFactors=FALSE)
...
#names(eegCleanDat)
meanEEG <- summarySE(eegDat, measurevar="F41", groupvars=c("cond")) # fnirsData
#describe(eegDat$low12) #describe(subset(rtm, cueInf == '50%')$reactionTime)
...
options(contrasts=c("contr.sum", "contr.poly"))
eegCleanDat$trial <- as.factor(eegCleanDat$trial)
ezANOVA(data=eegDat, dv=.(F41), wid=.(SubId), within=.(cond), type=3) 

ezANOVA(data=eegDat, dv=.(high15), wid=.(SubId), within=.(cond), type=3) 
